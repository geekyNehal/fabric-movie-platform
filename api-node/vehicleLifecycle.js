...
async function main() {
    try {
        console.log(`Chaincode running started ...`);
        // load the network configuration
        const ccpPath = path.resolve(__dirname, '..','vars', 'profiles', 'mychannel_connection_for_nodesdk.json');

        console.log(`Chaincode running 1 ...${ccpPath}`);
        let ccp = JSON.parse(fs.readFileSync(ccpPath, 'utf8'));
        console.log(`Chaincode running 2 CCP ...${ccp}`);
        ...
        await contract.submitTransaction('createCar','CAR10' ,'VW', 'Polo', 'Grey', 'Mary');
